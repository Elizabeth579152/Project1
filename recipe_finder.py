    ...
    if response.status_code == 200:
        ...
        if hits:
            for hit in hits:
                ...
                d[index] = [recipe_name, recipe_url, ingredients]   # adds to the dictionary so that all the choices have a unique number
                index += 1
        else:
...
def url_picker():
    ...
    if x.isdigit() and int(x) in d:
        ...
        query = session.query().filter(Recipe.recipe_name == table_entry[0])
        if not query:
            session.add(recipe)
            session.commit()
        print(table_entry[1])
